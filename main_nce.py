...
from utils.tools import AverageMeter, reduce_tensor, epoch_saving, load_checkpoint_fuse, generate_text, auto_resume_helper
...
from utils.config import get_config
from trainers import OST, vificlip
from einops import rearrange
from utils.textprompt import spatial_prompt, temporal_prompt, text_prompt
import warnings
import clip
from utils.NCELoss import NCELoss
...
def main(config):
    ...
    descriptor_spatial_token = spatial_prompt(train_data)
    A, N, W = descriptor_spatial_token.shape
    descriptor_spatial_token = rearrange(descriptor_spatial_token, 'a n w->(a n) w')
    descriptor_temporal_token = temporal_prompt(train_data)     
    descriptor_temporal_token = rearrange(descriptor_temporal_token, 'a n w->(a n) w')
    class_embedding_token = text_prompt(train_data)
        
    # Custom trainer
    model = OST.returnCLIP(config,
                          logger=logger,
                          des_spatial=descriptor_spatial_token,
                          des_temporal=descriptor_temporal_token,
                          class_emb=class_embedding_token)
    ...
    if config.MODEL.RESUME:
        start_epoch, max_accuracy = load_checkpoint_fuse(config, model, optimizer, lr_scheduler, logger)
        if start_epoch > 1:
    ...
    for epoch in range(start_epoch, config.TRAIN.EPOCHS):
        ...
        if (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)) and (epoch != 0):
            acc1 = validate(val_loader, model, config)
            logger.info(f"Accuracy of the network on the {len(val_data)} test videos: {acc1:.1f}%")
...
def train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, config, mixup_fn):
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        image_embedding, spatial_emb, temporal_emb, logit_scale = model(images, label_id)
        
        image_embedding = allgather(image_embedding)
        spatial_emb = allgather(spatial_emb)
        temporal_emb = allgather(temporal_emb)
        logits = model.module.get_logit(image_embedding, spatial_emb, temporal_emb, logit_scale)
        label_id = gather_labels(label_id)

        ground_truth = torch.tensor(gen_label(label_id),dtype=image_embedding.dtype,device=image_embedding.device)
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            for i in range(n):
                ...
                output = model.module.inference(image_input)
